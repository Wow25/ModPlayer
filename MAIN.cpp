...
void ApplyEffects(int e, uint16_t Type, uint8_t Value) {
    ...
    switch (Type) {
        ...
        case 15:
            ...
            if ((Value > 0) && (Value < 32)) {
                Speed = Value;
                SamplesPerTick = SampleRate * 0.02;
            } else {
    ...
    unsigned int i, j;
        ...
        for (i = 0; i < nBufferFrames; i++) { //Since a new note could come at any time, we have to do this
            ...
            for (int e = 0; e < 4; e++) {
                ...
                //Bad but why not
                if (!(N.SampleID == 255) && Channels[e].Playing && (Channels[e].EffectSlide == false)) {
                    Channels[e].SamplePointer = 0.0f;
                    Channels[e].ChannelVolume = SampleHeaders[N.SampleID].SampleVol;
                }
            } //Done
            ...
            if (TickCounter >= SamplesPerTick) { //882 is how many samples per tick (normally)
                ...
                if (TickNum == 0) {
                    ...
                    if (CurrentRow == 0) {
                        ...
                        CurrentPattern = PatternTable[PatternNumber];
                        std::cout << "New Pattern: " << (int)CurrentPattern;
                        if (PatternNumber >= NumOfPatterns) {
                            PatternNumber = 0;
                            CurrentPattern = PatternTable[PatternNumber];
                        }
                    }
...
int main()
{
    ...
    errno_t err;
    FILE* FilePoint;
    err = fopen_s(&FilePoint,"./AudioStuff/jogeir_liljedahl_-_endless.mod", "rb"); //rb is read binary
